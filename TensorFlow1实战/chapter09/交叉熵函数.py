...
# 运行程序,计算loss的结果是[0.32656264 0.4643688 ]
# 这对应两个预测的perplexity损失
sess = tf.Session()
print(sess.run(loss))

# softmax_cross_entropy_with_logits与上面的函数相似
# 但是需要将预测目标以概率分布的形式给出
...
# 由于softmax_cross_entropy_with_logits允许提供一个概率分布,因此在使用时有更大的
# 自由度。半个例子，一种叫 label smoothing 的技巧是将正确数据的概率设为一个比 1.0
# 略小的值，将错误数据的概率设为比 0.0 略大的值，这样可以避免模型与数据过拟合，在某些时候可以提高训练效果。
