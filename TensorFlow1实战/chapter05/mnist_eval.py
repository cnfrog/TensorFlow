...
def evaluate(mnist):
    ...
    # 通过变量的重命名的方式加载模型,这样在前向传播的过程中就不需要调用求滑动平均的函数来获取平均值了,这样就完全共用mnist_inference.py中定义的前向传播过程
    ...
    while True:
        with tf.Session() as sess:
            ...
            if ckpt and ckpt.model_checkpoint_path:
                ...
                print('After %s training step(s), accuracy is %g.' % (global_step, accuracy_score))
            else:
