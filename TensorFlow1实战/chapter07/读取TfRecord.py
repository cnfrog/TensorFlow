...
features = tf.parse_single_example(
    ...
    features={
        ...
        # 这种方法解析的结果是一个Tensor, 另一种方法是tf.VarLenFeature, 这种方法得到的解析结果
        # 是SparseTensor, 用于处理稀疏数据, 这里解析数据的格式需要和上面程序写入数据的格式一致.
...
# 每次运行可以读取TFRecord文件中的一个样例,当所有样例都读完之后,在此样例中程序会再重头再取
for i in range(10):
