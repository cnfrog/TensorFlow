...
# 加载mnist_interfere.py中定义的常量和前向传播的函数
...
def train(mnist):
    ...
    # 直接使用mnist_interfere.py中定义的前向传播过程
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i % 1000 == 0:
                ...
                print('After %d training step(s), loss on training batch is %g.' % (step, loss_vaule))
                # 保存当前模型,注意这里给出了global_step参数,这样可以让每个被保存模型的文件名末尾加上训练的轮数,比如model.ckpt-1000表示训练1000轮之后的模型
